...
export default function App() {
  ...
  async function generate() {
    ...
    try {
      const response = await fetch(`${API_URL}/generate-code`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...(await authHeaders()) }, body: JSON.stringify({ request, language, framework, level: 'junior', style: 'standard' }) })
  ...
  async function saveSnippet() {
    ...
    try {
      const response = await fetch(`${API_URL}/snippets`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...(await authHeaders()) }, body: JSON.stringify({ title: result?.title || 'Snippet sin titulo', prompt: request, code, language, framework, category: 'generated' }) })
  ...
  async function loadSnippets() {
    if (!session) return
    try {
      const response = await fetch(`${API_URL}/snippets`, { headers: await authHeaders() })
